...
class KnightsTour:
    ...
    def checkGameOver(self):
        if len(self.knight.validMoves) == 0 and len(self.moveStk) == self.endMoves:
            self.over = True
            return True
    ...
    def findSolution(self, pSolution):
        ...
        # get list of valid moves of the knight ordered by the Warnsdorff heuristic
        validMoves = sorted(self.knight.validMoves, key = lambda x : self.getHeuristic(x))

        for move in validMoves:  # try each valid move
            # make the move
            ...
            self.moveKnight(pos)  # move knight back to the previous move
